    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                {{ $usuario ? "Edición de Usuario" : "Registro de nuevo Usuario"}}
            </h6>
        </div>

        <!-- CREAR NUEVA EDICIÓN -->
        <div class="card-header py-3" style="border-bottom: 0px;">
            <a href="{{ route('usuario.index') }}" class="btn btn-secondary btn-icon-split">
                <span class="icon text-white-50">
                    <i class="fas fa-rotate-left"></i>
                </span>
                <span class="text">Regresar a todos los usuarios</span>
            </a>
        </div>
        ...
        <div class="card-body container">
            <div class="row">
                ...
                <div class="col-lg-6">
                    <form method="POST" enctype="multipart/form-data" action="{{ $usuario ? route('usuario.update', $usuario->id) : route('usuario.store') }}">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-8">
                                <input id="telefono" type="text" class="form-control @error('telefono') 'is-invalid' @enderror" name="telefono" 
                                        value="{{ $usuario ? old('telefono', substr($usuario->perfil->telefono, 3)) : old('telefono') }}" autofocus placeholder="Teléfono del Usuario...">
                            </div>
                            <div class="col-12">
                                @error('telefono')
                                    <span class="invalid-feedback" role="alert">
                        ...
                        <!-- Tipo de Usuario -->
                        <div class="form-group">
                            <label for="FK_id_tipo">Que tipo de Usuario es?</label>
                            <select id="FK_id_tipo" class="form-control @error('FK_id_tipo') is-invalid @enderror" name="FK_id_tipo" required>
                                <option value="">Selecciona un tipo de usuario...</option>
                                @foreach($tipos as $tipo)
                                    <option value="{{ $tipo->id_tipo }}" {{ old('FK_id_tipo', $usuario ? $usuario->perfil->FK_id_tipo : "") == $tipo->id_tipo ? "selected" : ""}}>{{ $tipo->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <!-- Rol del Usuario -->
                        <div class="form-group">
                            <label for="FK_id_rol">Que rol tiene el usuario en el sistema?</label>
                            <select id="FK_id_rol" class="form-control @error('FK_id_edicion') is-invalid @enderror" name="FK_id_rol" required>
                                <option value="">Selecciona una Edición...</option>
                                @foreach($roles as $rol)
                                    <option value="{{ $rol->id_rol }}" {{ old('FK_id_rol', $usuario ? $usuario->urol->FK_id_rol : "") == $rol->id_rol ? "selected" : ""}}>{{ $rol->nombre }}</option>
                                @endforeach
                            </select>
